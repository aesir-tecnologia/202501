-- Migration: Modify get_daily_summaries to return live data for today
--
-- Problem: Today's stints don't appear in AchievementCard because
-- daily_summaries table is only populated by a nightly cron job.
--
-- Solution: For today (in user's timezone), calculate live statistics
-- from stints table. For past dates, use pre-aggregated daily_summaries.
...
COMMENT ON FUNCTION get_daily_summaries(UUID, DATE, DATE) IS
  'Retrieves daily summaries for a user within a date range. Returns live-calculated data for today (in user timezone), pre-aggregated data for past dates.';
